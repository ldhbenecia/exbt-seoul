...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background dark">
        ...
        <div className="container mx-auto max-w-4xl text-center space-y-6 sm:space-y-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 pt-4 sm:pt-8 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300">
            <Button
              ...
              className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 text-sm sm:text-base font-normal rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground transition-all border-0"
            >
              전시회 둘러보기
            </Button>
          </div>
      ...
      {/* Exhibitions Section */}
      <main id="exhibitions-section" className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-24">
        {/* Filter Section */}
        <div className="mb-8 sm:mb-12 lg:mb-16 animate-in fade-in slide-in-from-bottom-6 duration-700">
          <FilterBar
            category={category}
        ...
        {!isLoading && !error && filteredExhibitions.length > 0 && (
          <div className="mb-6 sm:mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100">
            <p className="text-sm sm:text-base text-muted-foreground">
              <span className="font-medium text-foreground">{filteredExhibitions.length}개</span>의 전시회를 찾았습니다
            </p>
          </div>
        )}
        ...
        {isLoading && (
          <div className="text-center py-24 sm:py-32">
            ...
            <p className="text-muted-foreground mt-4 text-sm sm:text-base">전시회 정보를 불러오는 중...</p>
          </div>
        )}
        ...
        {error && (
          <div className="text-center py-24 sm:py-32">
            <p className="text-destructive text-base sm:text-lg font-medium">전시회 정보를 불러오는데 실패했습니다</p>
        ...
        {!isLoading && !error && filteredExhibitions.length === 0 && (
          <div className="text-center py-24 sm:py-32">
            ...
            <p className="text-muted-foreground/70 mt-2 text-sm">다른 검색어나 카테고리를 시도해보세요</p>
          </div>
        )}
